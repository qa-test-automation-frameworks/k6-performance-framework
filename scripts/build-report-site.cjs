...
const rows = reportData
  .map(
    ({ name, p95, p99, errorRate }) =>
      `<tr><td>${name}</td><td>${p95?.toFixed(2) ?? '-'}</td><td>${p99?.toFixed(2) ?? '-'}</td><td>${errorRate?.toFixed(4) ?? '-'}</td></tr>`,
  )
  .join('');
const evidence =
  rows || '<tr><td colspan="4">No measured performance runs are published yet.</td></tr>';
...
<table><thead><tr><th>Run</th><th>p95 ms</th><th>p99 ms</th><th>Error rate</th></tr></thead><tbody>${evidence}</tbody></table>
<script>
