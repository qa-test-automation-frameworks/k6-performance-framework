...
const summaries = fs.existsSync('reports')
  ...
  : [];
if (fs.existsSync('baseline/load-summary.json')) summaries.push('../baseline/load-summary.json');
...
<table><thead><tr><th>Run</th><th>p95 ms</th><th>p99 ms</th><th>Error rate</th></tr></thead><tbody>${rows}</tbody></table></body></html>`;
