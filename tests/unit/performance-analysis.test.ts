...
describe('performance summaries', () => {
  it('writes aggregate JSON and Markdown outputs', () => {
    const result = createSummary({
      metrics: {
        http_req_duration: { type: 'trend', contains: 'time', values: { 'p(95)': 400, 'p(99)': 700 } },
    ...
    expect(result['reports/k6-summary-summary.md']).toContain('| http_req_duration | 400.00 | 700.00 |');
  });
});
