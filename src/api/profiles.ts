...
export class ProfilesService {
  ...
  follow(token: string, username: string): HttpResponse<ProfileResponse> {
    return this.client.post(`/profiles/${encodeURIComponent(username)}/follow`,
      'POST /profiles/:username/follow', {}, { params: auth(token) });
  }

  unfollow(token: string, username: string): HttpResponse<ProfileResponse> {
    return this.client.delete(`/profiles/${encodeURIComponent(username)}/follow`,
      'DELETE /profiles/:username/follow', { params: auth(token) });
  }
}
